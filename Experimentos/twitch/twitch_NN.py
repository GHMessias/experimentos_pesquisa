...
for rate in positive_rate:
    for i in range(10):
        ...
        model_AE = AutoEncoder(input_size=X.shape[1], hidden_size1=36, hidden_size2=4)
        optimizer_AE = optim.Adam(model_AE.parameters(), lr=0.001)
        
        model_GAE1 = GraphAutoEncoder(input_dim=X.shape[1], hidden_dim1=36, hidden_dim2=4)
        optimizer_GAE1 = optim.Adam(model_GAE1.parameters(), lr=0.001)
        
        model_GAE2 = GraphAutoEncoder(input_dim=X.shape[1], hidden_dim1=36, hidden_dim2=4)
        optimizer_GAE2 = optim.Adam(model_GAE2.parameters(), lr=0.001)
        
        model_GAE3 = GraphAutoEncoder(input_dim=X.shape[1], hidden_dim1=36, hidden_dim2=4)
        optimizer_GAE3 = optim.Adam(model_GAE3.parameters(), lr=0.001)
        
        model_GAE4 = GraphAutoEncoder(input_dim=X.shape[1], hidden_dim1=36, hidden_dim2=4)
...
df.to_csv('Resultados/dataframe_twitch_NN.csv', index = False)