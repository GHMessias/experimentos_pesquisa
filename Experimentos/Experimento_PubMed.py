...
for rate in positive_rate:
    for i in range(10):
        ...
        'PU_LP' : PU_LP(data = X, positives = positives, unlabeled = unlabeled, alpha = 0.3, m = 10, l = 5),
        'RCSVM_RN' : RCSVM_RN(data = X, positives = positives, unlabeled = unlabeled, alpha = 0.1, beta = 0.9)
        }
        for algorithm in algorithms_pubmed:
            print('dataset: PubMed')           
            start_time = time.time()
            print(f'algoritmo {algorithm}, porcentagem do dataset positivo {rate}')
            algorithms_pubmed[algorithm].train()
            # if algorithm in auto_inference_algorithms:
            #     RN = algorithms_pubmed[algorithm].negative_inference()
            # else:
            RN = algorithms_pubmed[algorithm].negative_inference(num_neg)
            end_time = time.time()
