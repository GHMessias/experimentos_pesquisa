...
dataset = Planetoid(root = "Datasets", name = "PubMed", transform=NormalizeFeatures())
data = dataset[0]
...
for rate in positive_rate:
    for i in range(10):
        ...
        model_AE = AutoEncoder(input_size=X.shape[1], hidden_size1=64, hidden_size2=16)
        model_GAE1 = GraphAutoEncoder(input_dim=X.shape[1], hidden_dim1=64, hidden_dim2=16)
        model_GAE2 = GraphAutoEncoder(input_dim=X.shape[1], hidden_dim1=64, hidden_dim2=16)
        model_GAE3 = GraphAutoEncoder(input_dim=X.shape[1], hidden_dim1=64, hidden_dim2=16)
        ...
        for algorithm in algorithms:
            ...
            df.to_csv('Resultados/dataframe_PubMed.csv', index = False)
